...
| 실험명 | 비교 축 | emb_dim | n_layers | drop_rate | lr | epoch | LM train loss | LM val loss | sentiment val loss | sentiment val acc | sentiment test acc | 생성 샘플 특징 | 학습 시간 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| Model-Small | model_size | 128 | 2 | 0.1 | 3e-4 | 3 | 5.8341 | 5.9592 | 0.6467 | 0.634 | 0.607 | 영화 리뷰 어휘는 나오지만 문장 연결이 불안정함 | 미기록(JSON 미포함) |
| Model-Base | model_size | 192 | 4 | 0.1 | 3e-4 | 3 | 5.2563 | 5.4527 | 0.6251 | 0.642 | 0.645 | 리뷰 문장 형태가 가장 안정적으로 나타남 | 미기록(JSON 미포함) |
| Model-Large | model_size | 256 | 4 | 0.1 | 3e-4 | 1 | 6.2821 | 6.3668 | 0.6683 | 0.588 | 0.571 | 큰 모델이지만 epoch가 짧아 충분히 수렴하지 못함 | 미기록(JSON 미포함) |
| LR-Low | learning_rate | 192 | 4 | 0.1 | 1e-4 | 3 | 6.4360 | 6.5452 | 0.6642 | 0.609 | 0.583 | loss 감소가 느리고 생성 문장이 불안정함 | 미기록(JSON 미포함) |
| LR-Base | learning_rate | 192 | 4 | 0.1 | 3e-4 | 3 | 5.2039 | 5.4535 | 0.6509 | 0.621 | 0.620 | baseline으로 안정적인 감소를 보임 | 미기록(JSON 미포함) |
| LR-High | learning_rate | 192 | 4 | 0.1 | 5e-4 | 1 | 6.1087 | 6.2297 | 0.6658 | 0.594 | 0.620 | 빠른 update를 기대했지만 val acc는 낮음 | 미기록(JSON 미포함) |
| Dropout-0.0 | dropout | 128 | 4 | 0.0 | 3e-4 | 3 | 5.3692 | 5.6573 | 0.6444 | 0.616 | 0.618 | train/val 간 차이가 생기며 일반화 이득은 제한적 | 미기록(JSON 미포함) |
| Dropout-0.2 | dropout | 128 | 4 | 0.2 | 3e-4 | 3 | 5.9743 | 6.0769 | 0.6774 | 0.533 | 0.553 | dropout이 커지며 학습 신호가 약해진 것으로 보임 | 미기록(JSON 미포함) |
...
Basic 사전 학습 결과는 `results/LR-Low2/pretrain_basic.json`을 기준으로 정리했다.
...
| drop_rate | 0.1 |
| 마지막 global step | 800 |
| 마지막 eval train loss | 6.8260 |
| 마지막 eval val loss | 6.8628 |
| Basic sentiment 결과 | 미실행/미제공(JSON 없음) |
| 학습 시간 | 미기록(JSON 미포함) |
...
중요한 점은 Basic 실험이 Light 실험의 결론을 그대로 확장했다는 것이다. 먼저 작은 설정에서 빠르게 후보를 비교하고, 그중 downstream validation 기준으로 가장 안정적인 조합을 더 큰 설정에 적용했다. 이것이 실험을 무작위 튜닝이 아니라 단계적 탐색으로 볼 수 있는 이유다.
...
| 결과 파일 | val loss | val acc | test loss | test acc |
| --- | ---: | ---: | ---: | ---: |
| `results/sentiment_light.json` | 0.6505 | 0.638 | 0.6235 | 0.657 |

두 결과 모두 Base 계열 설정이 validation accuracy 기준으로 안정적이라는 결론을 뒷받침한다.
...
- Basic sentiment 결과 JSON이 없어 Basic 설정에서의 최종 감성 분류 성능은 확인하지 못했다.
...
- Basic 설정에서도 sentiment fine-tuning을 실행해 Light에서 선택한 조합이 실제 최종 제출 설정에서도 이어지는지 확인해야 한다.
...
> 이번 mini GPT에서는 Light 설정 기준으로 너무 작은 모델은 표현력이 부족하고, 너무 큰 모델은 제한된 학습 조건에서 충분히 수렴하지 못했다. 또한 learning rate와 dropout은 극단적인 값보다 중간값이 validation 성능에서 안정적이었다. 그래서 `emb_dim=192`, `n_layers=4`, `lr=3e-4`, `drop_rate=0.1`을 최종 조합으로 선택했고, 이를 Basic 설정에 확장 적용했다.
